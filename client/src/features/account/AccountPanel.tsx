      ...
      <section className="account-panel-dialog" role="dialog" aria-label="Account settings">
        <header className="account-shell-header">
          <div className="account-header-info">
            <p className="account-eyebrow">Groundwork</p>
  ...
  <X size={18} />
        ...
        <div className="account-layout">
          ...
          <main className="account-settings-main">
            ...
            {tab === "profile" && (
              <>
                ...
                <div className="settings-grid profile-settings-grid">
                  ...
                  <section className={`settings-card identity-card ${user.google_linked ? "linked" : ""}`}>
                    <CheckCircle2 size={21} />
                    <div>
                      <h3>Google account</h3>
                      <p>{user.google_linked ? "Connected for secure sign-in" : "Not connected to this account"}</p>
                    </div>
                    <strong>{user.google_linked ? "Connected" : "Not linked"}</strong>
                  </section>
                </div>
                <form className="settings-card settings-form" onSubmit={updateProfile}>
                  <h3>Personal information</h3>
                  <p>This name appears in shared workspaces and document activity.</p>
                  <div className="settings-fields">
                    <label>
                      Display name
                      <input name="display_name" defaultValue={user.display_name} minLength={2} required />
                    </label>
                    <label>
                      Email address
            ...
            {tab === "security" && (
              <>
                <Heading eyebrow="Security" title="Password and sessions" detail="Control access to your account and remove sessions you no longer recognize." />
            ...
            {tab === "defaults" && (
              <>
                <Heading eyebrow="Document defaults" title="Start every document your way" detail="These defaults guide new AI drafts and exports. You can still change them per document." />
                <section className="settings-card settings-form">
                  <div className="settings-fields three">
                    ...
                    <label>
                      Default tone
                      <select value={preferences.default_tone} onChange={(event) => setPreferences({ ...preferences, default_tone: event.target.value as UserPreferences["default_tone"] })}>
                    ...
                    <label>
                      Citation style
                      <select value={preferences.citation_style} onChange={(event) => setPreferences({ ...preferences, citation_style: event.target.value as UserPreferences["citation_style"] })}>
                    ...
                    <label>
                      Page size
                      <select value={preferences.page_size} onChange={(event) => setPreferences({ ...preferences, page_size: event.target.value as UserPreferences["page_size"] })}>
                    ...
                    <label>
                      Default export
                      <select value={preferences.default_export_format} onChange={(event) => setPreferences({ ...preferences, default_export_format: event.target.value as UserPreferences["default_export_format"] })}>
            ...
            {tab === "notifications" && (
              <>
                <Heading eyebrow="Notifications" title="Stay on top of background work" detail="Groundwork keeps a durable inbox, so you can leave a task and return when it is ready." />
                <section className="settings-card toggle-list">
                  <Toggle checked={preferences.notify_processing_completed} onChange={(value) => setPreferences({ ...preferences, notify_processing_completed: value })} title="Completed processing and exports" detail="Tell me when uploads, AI tasks, and generated files are ready." />
                  <Toggle checked={preferences.notify_processing_failed} onChange={(value) => setPreferences({ ...preferences, notify_processing_failed: value })} title="Failures that need attention" detail="Show errors and a direct path back to processing details." />
            ...
            {tab === "privacy" && (
              <>
                <Heading eyebrow="Privacy & data" title="Your data, under your control" detail="Download your account data, manage activity retention, or permanently delete the account." />
                <section className="settings-card toggle-list">
                  <Toggle checked={preferences.retain_activity_history} onChange={(value) => setPreferences({ ...preferences, retain_activity_history: value })} title="Keep activity history" detail="Retain document activity so workspace progress remains auditable." />
                  <label className="inline-setting">
                    Retain history for
                    <select value={preferences.retention_days} onChange={(event) => setPreferences({ ...preferences, retention_days: Number(event.target.value) })}>
            ...
            {tab === "usage" && (
              <>
                <Heading eyebrow="Usage" title="Account activity at a glance" detail="See the files, storage, AI requests, and background work attached to your account." />
                <div className="account-stats usage-cards">
                  {stats && (
                    <>
                      <article><strong>{stats.document_count}</strong><span>Documents</span></article>
                      <article><strong>{stats.page_count}</strong><span>Pages indexed</span></article>
                      <article><strong>{stats.generated_files}</strong><span>Generated files</span></article>
                      <article className={stats.failed_jobs ? "warn" : ""}><strong>{stats.failed_jobs}</strong><span>Failed jobs</span></article>
                    </>
                  )}
                </div>
                {usage && (
                  <>
                    <section className="settings-card usage-storage">
                      <div>
                        <h3>Storage</h3>
                        <strong>{(usage.storage_bytes / 1024 / 1024).toFixed(1)} MB of {(usage.storage_limit_bytes / 1024 / 1024 / 1024).toFixed(0)} GB</strong>
                      </div>
                      <div className="usage-bar"><i style={{ width: `${storagePercent}%` }} /></div>
                      <small>{storagePercent}% used</small>
                    </section>
                    <div className="settings-grid">
                      <section className="settings-card">
                        <h3>AI activity</h3>
                        <div className="metric-emphasis"><strong>{usage.ai_requests_30_days}</strong><span>requests in the last 30 days</span></div>
                        <div className="usage-breakdown">
                          {Object.entries(usage.ai_requests_by_feature).sort((a, b) => b[1] - a[1]).map(([key, value]) => (
                            <p key={key}><span>{key.replaceAll("_", " ")}</span><strong>{value}</strong></p>
                          ))}
                          {!Object.keys(usage.ai_requests_by_feature).length && <p>No AI activity yet.</p>}
                        </div>
                      </section>
                      <section className="settings-card">
                        <h3>Background jobs</h3>
                        <div className="usage-breakdown">
                          {Object.entries(usage.jobs_by_status).map(([key, value]) => (
                            <p key={key}><span className={`job-state ${key}`}>{key}</span><strong>{value}</strong></p>
                          ))}
                          {!Object.keys(usage.jobs_by_status).length && <p>No jobs yet.</p>}
                        </div>
                      </section>
                    </div>
                  </>
            ...
            {tab === "team" && (
              <>
                <Heading eyebrow="Team" title="Workspace access" detail="Invite existing Groundwork users and control what they can change." />
                <section className="settings-card settings-form">
                  <label>
                    Workspace
                ...
                <section className="settings-card">
                  <div className="settings-card-header">
                    <div>
                      <h3>Members</h3>
                      <p>{members.length} people can access this workspace.</p>
                    </div>
                  </div>
                  <div className="settings-list team-list">
                    {members.map((member) => (
                      <article key={member.id}>
                        <div className="member-avatar">{member.display_name.slice(0, 2).toUpperCase()}</div>
                        <span><strong>{member.display_name}{member.user_id === user.id ? " (you)" : ""}</strong><small>{member.email}</small></span>
                        {member.role === "owner" || !editableTeam ? (
                          <b>{member.role}</b>
                        ) : (
                          <>
                            <select value={member.role} onChange={(event) => perform(async () => { const updated = await api<WorkspaceMember>(`/workspaces/${workspaceId}/members/${member.id}`, token, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ role: event.target.value }) }); setMembers((current) => current.map((item) => item.id === updated.id ? updated : item)); }, "Member access updated.")}>
            ...
            {tab === "admin" && user.role === "admin" && (
              <>
                <Heading eyebrow="Admin" title="User management" detail="Review account activity and disable access when necessary." />
                <section className="settings-card admin-users">
                  <div className="settings-card-header">
                    <div>
                      <h3>All users</h3>
                      <p>{admins.length} registered accounts</p>
                    </div>
                  </div>
                  {admins.map((item) => (
                    <article key={item.id}>
                      <div className="member-avatar">{item.display_name.slice(0, 2).toUpperCase()}</div>
                      <div>

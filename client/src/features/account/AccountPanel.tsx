...
export function AccountPanel({ user, token, stats, onUser, onClose, onSignOut }: {
  ...
  async function exportData() {
    await perform(async () => {
      ...
      downloadTextFile(`insightpdf-account-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(value, null, 2), "application/json");
  ...
  return <div className="account-wrap"><button className="history-backdrop" aria-label="Close account settings" onClick={onClose} />
    <section className="account-panel account-panel-full" role="dialog" aria-label="Account settings">
      <header className="account-shell-header"><div><p>InsightPDF account</p><strong>{user.display_name}</strong><span>{user.email}</span></div><button aria-label="Close account settings" onClick={onClose}><X size={19} /></button></header>

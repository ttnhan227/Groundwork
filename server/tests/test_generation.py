            ...
            "sections": [
                {
                    ...
                    "body": "Professional fictional invoice information for testing dynamic layouts.",
                }
    ...
    plan = GeneratedContent.model_validate(
        {
            ...
            "sections": [
                {
                    "heading": f"Section {index}",
                    "body": "Specific evidence-backed operating content for executive review.",
                }
                for index in range(1, 7)
            ],
...
def test_unsupported_generated_metrics_are_marked_for_verification() -> None:
    plan = GeneratedContent.model_validate(
        {
            ...
            "metadata": [],
            "sections": [
                {"heading": "Performance", "body": "Retention improved by 22% and the 12-week rollout is complete."}
            ],
    ...
    verified = GeneratedContent.model_validate(
        {
            ...
            "accent_color": "#5B5CE2",
            "metadata": [],
            "sections": [{"heading": "Release", "body": "We launched the beta program."}],
            "table": None,
            "callout": "",
        }
    )
    assert (
        "[confirm claim]"
        not in _guard_unsupported_metrics(verified, "We launched the beta program in June.").sections[0].body
    )
